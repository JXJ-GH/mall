package DAO.commodityImpl;



import DAO.commodityDao.ICommodityDao;
...
public class CommodityDaoImpl implements ICommodityDao {
    ...
    public void insert(CommodityIfo commodityIfo) {
        try {
            ...
            String sql = "insert into mmall_product (name,price,stock,capacity,sub_images,years,degree,distributor,sort) values(?,?,?,?,?,?,?,?,?)";
            ...
            sta.setInt(9, commodityIfo.getCommoditySort());
            sta.execute();
    ...
    public void upDate(CommodityIfo commodityIfo) {
        ...
        try {
            conn = JdbcUtils.getConnection();
            String sql = "update mmall_product set name=?,price=?,stock=?,capacity=?,sub_images=?,years=?,degree=?,distributor=?,sort=? where id = ?";
            sta = conn.prepareStatement(sql);
            sta.setString(1, commodityIfo.getCommodityName());
            sta.setDouble(2, commodityIfo.getCommodityPrice());
            sta.setInt(3, commodityIfo.getCommodityStock());
    ...
    public CommodityIfo selectByID(int commodityID) {
        ...
        try {
            conn = JdbcUtils.getConnection();
            String sql = "select name,price,stock,capacity,sub_images,years,degree,distributor,sort from mmall_product where id= ?";
            sta = conn.prepareStatement(sql);
            sta.setInt(1, commodityID);
            rs = sta.executeQuery();
            while (rs.next()) {
    ...
    public List<CommodityIfo> fuzzySelect(String str) {
        ...
        try {
            ...
            conn = JdbcUtils.getConnection();
            String sql = "select id,name,price,stock,capacity,sub_images,years,degree,distributor,sort from mmall_product where name like '%"
